    ...
    def __new__(cls, name, bases, attrs):
        ...
        for attrname, attrvalue in attrs.items():
            if attrname.startswith("test") and not getattr(
                ...
                # Expand test variants (e.g. swift clang/noclang) only on the
                # entries we just created for this attrname, not on the whole
                # newattrs dict (which would double-expand sibling methods
