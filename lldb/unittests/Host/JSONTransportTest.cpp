...
TEST_F(TransportBinderTest, InBoundAsyncRequests) {
  ...
  EXPECT_CALL(remote, Received(Resp{1, 0, MyFnResult{7}}));
  Run();
}
...
TEST_F(TransportBinderTest, InBoundAsyncRequestsVoidParams) {
  ...
  EXPECT_THAT_ERROR(from_remote->Send(Req{2, "ping", std::nullopt}),
                    Succeeded());
  Run();
  ASSERT_TRUE(static_cast<bool>(saved_reply));

  saved_reply(MyFnResult{5});
  EXPECT_CALL(remote, Received(Resp{2, 0, MyFnResult{5}}));
  Run();
}
...
TEST_F(TransportBinderTest, InBoundAsyncRequestsError) {
  ...
  EXPECT_THAT_ERROR(from_remote->Send(Req{3, "add", MyFnParams{1, 2}}),
                    Succeeded());
  EXPECT_CALL(remote, Received(Resp{3, 1, "nope"}));
  Run();
}
