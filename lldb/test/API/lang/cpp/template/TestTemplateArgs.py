...
class TemplateArgsTestCase(TestBase):
    ...
    def test_integer_args(self):
        ...
        self.assertEqual(expr_result.GetValue(), "-1", "testneg.getArg() == -1")
        self.assertEqual(
            expr_result.GetType().GetName(),
            "int",
            'expr_result.GetType().GetName() == "int"',
    ...
    def test_enum_args(self):
        ...
        self.assertTrue(
            member.IsValid(), 'make sure we find a local variabble named "member"'
        )
        self.assertEqual(member.GetType().GetName(), "EnumTemplate<EnumType::Member>")
        ...
        self.assertEqual(expr_result.GetValue(), "123", "member.getMember() == 123")
        self.assertEqual(
            expr_result.GetType().GetName(),
            "int",
            'expr_result.GetType().GetName() == "int"',
        ...
        self.assertTrue(
            subclass.IsValid(), 'make sure we find a local variabble named "subclass"'
        )
        self.assertEqual(
            subclass.GetType().GetName(), "EnumTemplate<EnumType::Subclass>"
        )
        ...
        self.assertEqual(expr_result.GetValue(), "246", "subclass.getMember() == 246")
        self.assertEqual(
            expr_result.GetType().GetName(),
            "int",
            'expr_result.GetType().GetName() == "int"',
