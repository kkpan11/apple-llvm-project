...
bool lldb_private::formatters::swift::Duration_SummaryProvider(
  ...
  // Capture high/low
  ValueObjectSP high_sp(valobj.GetChildMemberWithName("_high"));
  ValueObjectSP low_sp(valobj.GetChildMemberWithName("_low"));

  if (!high_sp || !low_sp)
    return false;

  bool success = false;
  int64_t high = high_sp->GetValueAsSigned(0, &success);
  if (!success)
    return false;
  uint64_t low = low_sp->GetValueAsUnsigned(0, &success);
  if (!success)
    return false;

  // Building 128 bit value
  __uint128_t high_128 = static_cast<__uint128_t>(high);
  __uint128_t low_128 = static_cast<__uint128_t>(low);

  __uint128_t container = (high_128 << 64) | low_128;
  __int128_t duration = static_cast<__int128_t>(container);

  bool is_negative = duration < 0;
  if (is_negative)
    duration = -duration;

  __int128_t ATTO = 1000000000000000000; // 10^18
  __uint128_t seconds_128 = duration / ATTO;
  __uint128_t fraction_128 = duration % ATTO;
  ...
  if (is_negative)
    stream.PutChar('-');
  stream.Printf("%s.%s seconds", seconds_str.c_str(), fraction_str.c_str());
