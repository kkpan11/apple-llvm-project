...
bool lldb_private::formatters::IsMsvcStlStringViewType(ValueObject &valobj) {
  std::vector<uint32_t> indexes;
  ExecutionContext exe_ctx(valobj.GetExecutionContextRef());
  return valobj.GetCompilerType().GetIndexOfChildMemberWithName("_Mydata", &exe_ctx, 
                                                                true, indexes) > 0;
}
...
bool lldb_private::formatters::IsMsvcStlOrdering(ValueObject &valobj) {
  std::vector<uint32_t> indexes;
  ExecutionContext exe_ctx(valobj.GetExecutionContextRef());
  return valobj.GetCompilerType().GetIndexOfChildMemberWithName(
             "_Value", &exe_ctx, true, indexes) > 0;
}
