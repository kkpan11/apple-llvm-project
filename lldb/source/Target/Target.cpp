...
bool Target::RunStopHooks(bool at_initial_stop) {
  ...
  if (m_stop_hooks.empty() && m_hooks.empty())
    return false;

  bool no_active_stop_hooks =
  ...
  if (no_active_stop_hooks && active_unified_hooks.empty())
    return false;
  ...
  bool print_hook_header =
      (m_stop_hooks.size() + active_unified_hooks.size()) != 1;
