...
static CASKind CASSelection;
static std::string CASBuiltinPath;
...
static void parseRawArgs(int Argc, char **Argv) {
  ...
  if (const opt::Arg *A = Args.getLastArg(OPT_casSelection))
    CASSelection = llvm::StringSwitch<CASKind>(A->getValue())
                       .Case("builtin", CASKind::Builtin)
                       .Case("plugin", CASKind::Plugin);

  if (const opt::Arg *A = Args.getLastArg(OPT_casBuiltinPath))
    CASBuiltinPath = A->getValue();
