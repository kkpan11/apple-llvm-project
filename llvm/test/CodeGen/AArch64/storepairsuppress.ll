...
; RUN: llc -mtriple=aarch64 -mcpu=apple-m1 -o - %s | FileCheck --check-prefix=CHECK %s

define void @vector_units_critial(ptr %dst, <8 x i32> %v0) {
; CHECK-LABEL: vector_units_critial:
; CHECK:       // %bb.0: // %entry
...
; CHECK-LABEL: load_store_units_critical:
; CHECK:       // %bb.0: // %bb
; CHECK-NEXT:    ldr x8, [x0, #8]
; CHECK-NEXT:    ldp s0, s1, [x8]
; CHECK-NEXT:    add x8, x1, x2, lsl #3
; CHECK-NEXT:    ldp s2, s3, [x8]
; CHECK-NEXT:    ldp s4, s5, [x8, #8]
; CHECK-NEXT:    fmul s6, s5, s1
; CHECK-NEXT:    fmul s1, s4, s1
; CHECK-NEXT:    fnmsub s4, s4, s0, s6
...
; CHECK-NEXT:    ldp s16, s17, [x8, #24]
; CHECK-NEXT:    fmul s18, s17, s4
; CHECK-NEXT:    fmul s4, s16, s4
; CHECK-NEXT:    fnmsub s16, s16, s3, s18
...
; CHECK-NEXT:    fmul s17, s7, s17
; CHECK-NEXT:    fnmsub s7, s7, s4, s18
...
; CHECK-NEXT:    ldp s1, s4, [x10]
; CHECK-NEXT:    fmul s5, s4, s3
; CHECK-NEXT:    fmul s3, s1, s3
; CHECK-NEXT:    fnmsub s1, s1, s6, s5
...
; CHECK-NEXT:    ldp s16, s17, [x8, #56]
; CHECK-NEXT:    fmul s18, s17, s2
; CHECK-NEXT:    fmul s2, s16, s2
; CHECK-NEXT:    fnmsub s16, s16, s0, s18
...
; CHECK-NEXT:    fmul s17, s3, s17
; CHECK-NEXT:    fnmsub s3, s3, s2, s18
...
; CHECK-NEXT:    fmul s7, s5, s0
; CHECK-NEXT:    fmul s0, s1, s0
; CHECK-NEXT:    fnmsub s1, s1, s16, s7
...
; CHECK-NEXT:    fmul s20, s18, s5
; CHECK-NEXT:    fmul s5, s17, s5
; CHECK-NEXT:    fnmsub s17, s17, s4, s20
...
; CHECK-NEXT:    ldp s4, s5, [x9]
; CHECK-NEXT:    ldp s6, s17, [x8, #8]
; CHECK-NEXT:    fmul s18, s16, s5
; CHECK-NEXT:    fmul s5, s7, s5
; CHECK-NEXT:    fnmsub s7, s7, s4, s18
...
; CHECK-NEXT:    ldp s4, s5, [x9]
; CHECK-NEXT:    ldp s6, s7, [x8, #16]
; CHECK-NEXT:    fmul s16, s3, s5
; CHECK-NEXT:    fmul s5, s2, s5
; CHECK-NEXT:    fnmsub s2, s2, s4, s16
...
; CHECK-NEXT:    ldr x10, [x0, #8]
; CHECK-NEXT:    add x9, x10, x9, asr #29
; CHECK-NEXT:    ldp s2, s3, [x9]
; CHECK-NEXT:    ldp s4, s5, [x8, #24]
; CHECK-NEXT:    fmul s6, s1, s3
; CHECK-NEXT:    fmul s3, s0, s3
; CHECK-NEXT:    fnmsub s0, s0, s2, s6
