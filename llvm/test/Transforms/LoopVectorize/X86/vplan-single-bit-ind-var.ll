...
; CHECK-NEXT:  [[HEADER:.*:]]
; CHECK-NEXT:    br i1 false, label %{{.*}}, label %[[VECTOR_PH:.*]]
; CHECK:       [[VECTOR_PH]]:
...
; CHECK-NEXT:    [[TMP8:%.*]] = getelementptr float, ptr [[BAR]], i32 0
; CHECK-NEXT:    store <2 x float> [[TMP7]], ptr [[TMP8]], align 4
; CHECK-NEXT:    br label %[[MIDDLE_BLOCK:.*]]
; CHECK:       [[MIDDLE_BLOCK]]:
; CHECK-NEXT:    br i1 true, label %[[EXIT:.*]], label %{{.*}}
; CHECK:       [[EXIT]]:
; CHECK-NEXT:    ret void
;
header:
