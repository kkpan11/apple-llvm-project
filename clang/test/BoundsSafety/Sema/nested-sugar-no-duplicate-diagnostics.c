...
// count/bound attribute. This leaf path returns "invalid" on the bare-pointer
// layer, so the conflict is reported exactly once.
...
// layers. Unlike the conflict above, this leaf path emits and then *continues*
// (recovering by treating the count as a byte count), so the recovery must
// suppress a repeat of the diagnostic on the later desugar layers. Only the
// counted_by family reaches this path: __sized_by counts bytes (valid on
// 'void *'), and __ended_by is a range attribute.
typedef void * vp_t;
typedef vp_t vp2_t;
typedef vp2_t vp3_t;
