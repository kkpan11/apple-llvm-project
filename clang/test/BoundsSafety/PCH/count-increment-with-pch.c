...
// NOTE: `--include-generated-funcs` (in UTC_ARGS above) is required here: `foo()`
// is defined in the precompiled header (the `#ifndef HEADER` branch), so when
// `%s` is compiled with `-include-pch` it is not a top-level decl of the main
// translation unit. The JSON AST dump that `update_cc_test_checks.py` uses to
// anchor CHECK lines therefore never sees `foo`, and without this flag the tool
// silently drops `@foo`'s checks even though `@foo` (with its bounds checks) is
// in the emitted IR that FileCheck runs against.
...
// CHECK-NEXT:    [[SUB_PTR_LHS_CAST:%.*]] = ptrtoaddr ptr [[WIDE_PTR_UB]] to i64, !annotation [[META2]]
// CHECK-NEXT:    [[SUB_PTR_RHS_CAST:%.*]] = ptrtoaddr ptr [[WIDE_PTR_PTR]] to i64, !annotation [[META2]]
...
// CHECK-NEXT:    br i1 [[TMP5]], label [[CONT10:%.*]], label [[TRAP9:%.*]], !prof [[PROF1]], !annotation [[META2]]
// CHECK:       trap9:
