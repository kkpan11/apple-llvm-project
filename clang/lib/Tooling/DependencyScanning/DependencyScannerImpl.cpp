...
struct AsyncModuleCompile : PPCallbacks {
  ...
  void moduleLoadSkipped(Module *M) override {
    ...
    if (Service.getFormat() == ScanningOutputFormat::IncludeTree ||
        Service.getFormat() == ScanningOutputFormat::FullIncludeTree)
      VFS = llvm::cas::createCASProvidingFileSystem(Service.getCAS(),
                                                    std::move(VFS));
...
bool DependencyScanningAction::runInvocation(
  ...
  initializeScanCompilerInstance(ScanInstance, FS, DiagConsumer, Service, DepFS,
                                 VerboseOS);
...
bool CompilerInstanceWithContext::initialize(DiagnosticConsumer *DC) {
  ...
  CIPtr = std::make_unique<CompilerInstance>(
      createScanCompilerInvocation(*OriginalInvocation, Worker.Service, false),
  ...
  initializeScanCompilerInstance(
      ...
      Worker.Service, Worker.DepFS, nullptr);
