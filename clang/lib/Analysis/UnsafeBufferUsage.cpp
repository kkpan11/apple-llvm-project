...
static StringRef getExprTextOr(const Expr *E, const SourceManager &SM,
  ...
  std::optional<Token> EndToken =
      Lexer::findNextToken(E->getEndLoc().getLocWithOffset(-1), SM, LangOpts);
...
// Return the source location of the last character of `Node`.
template <typename NodeTy>
static std::optional<SourceLocation> getEndCharLoc(const NodeTy *Node,
                                                  const SourceManager &SM,
                                                  const LangOptions &LangOpts) {
  std::optional<Token> LastToken = Lexer::findNextToken(
      Node->getEndLoc().getLocWithOffset(-1), SM, LangOpts);

  if (!LastToken)
    return std::nullopt;
  return LastToken->getEndLoc().getLocWithOffset(-1);
}
...
populateInitializerFixItWithSpan(const Expr *Init, const ASTContext &Ctx,
  ...
  if (auto CxxNew = dyn_cast<CXXNewExpr>(Init->IgnoreImpCasts())) {
    ...
    if (const Expr *Ext = CxxNew->getArraySize().value_or(nullptr)) {
      if (!Ext->HasSideEffects(Ctx))
        ExtentText =
            getExprTextOr(Ext, SM, LangOpts, UserFillPlaceHolder);
  ...
  std::optional<SourceLocation> LocPassInit = getEndCharLoc(Init, SM, LangOpts);

  if (!LocPassInit)
    return {};
  // One past the last character of `Init`:
  LocPassInit = LocPassInit->getLocWithOffset(1);
  StrBuffer.append(", ");
...
static FixItList fixVarDeclWithSpan(const VarDecl *D, const ASTContext &Ctx,
  ...
  } else if (auto DeclEndCharLoc = getEndCharLoc(D, SM, Ctx.getLangOpts()))
    // The loc right before the loc right passed `D`:
    ReplacementLastLoc = *DeclEndCharLoc;
  else
