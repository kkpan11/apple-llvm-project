...
void CodeGenFunction::EmitTrapCheck(llvm::Value *Checked,
  ...
  if (!ClSanitizeDebugDeoptimization &&
      ...
      !CGM.getCodeGenOpts().UniqueTrapBlocks) {
      /*TO_UPSTREAM(BoundsSafety) OFF*/
    ...
    if (CGM.getCodeGenOpts().TrapFuncReturns) {
      ...
      auto *TrapID = llvm::ConstantInt::get(
          CGM.Int8Ty, (ClSanitizeDebugDeoptimization &&
                       !CGM.getCodeGenOpts().UniqueTrapBlocks)
                          ? TrapBB->getParent()->size()
                          : static_cast<uint64_t>(CheckHandlerID));
      TrapCall = Builder.CreateCall(CGM.getIntrinsic(llvm::Intrinsic::ubsantrap), TrapID);
