...
/// UniquingSet info for the PointerType pool, keyed on a QualType and the
/// bounds-safety pointer attributes.
struct PointerTypeInfo {
  using KeyTy = std::pair<QualType, BoundsSafetyPointerAttributes>;

  template <typename T> static KeyTy getKey(const T &N) { return N.getKey(); }

  static unsigned getHashValue(const KeyTy &Key) {
    return llvm::hash_combine(Key.first.getAsOpaquePtr(),
                              Key.second.getAsOpaqueValue());
  }
...
class ASTContext : public RefCountedBase<ASTContext> {
  ...
  mutable llvm::UniquingSet<PointerType, PointerTypeInfo> PointerTypes{
