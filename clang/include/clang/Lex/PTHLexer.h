...
class PTHLexer : public PreprocessorLexer {
  ...
  /// Pointer into current offset of the token buffer where the next token will
  /// be read.
  const unsigned char *CurPtr;

  /// Pointer into TokBuf of the last processed '#' token that appears at the
  /// start of a line.
  const unsigned char *LastHashTokPtr = nullptr;

  /// Pointer to a side table in the PTH file that provides a a concise summary
  /// of the preprocessor conditional block structure.  This is used to perform
  /// quick skipping of conditional blocks.
  const unsigned char *PPCond;

  /// Pointer inside PPCond that refers to the next entry to process when doing
  /// quick skipping of preprocessor blocks.
  const unsigned char *CurPPCondPtr = nullptr;
  ...
  IdentifierInfo *getOrCreateIdentifier(uint32_t IdentifierID, Token &T);
  ...
  PTHLexer(PTHHandler &Handler, Preprocessor &PP, FileID FID);
